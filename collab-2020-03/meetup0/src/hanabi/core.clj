...
(defn discard-card [{:keys [discard clue-avail clue-used] :as game-state} card]
  (prn :discard card)
  (merge game-state
...
(defn player-first-card [{:keys [hands] :as game-state}]
...
(defn next-hand [{:keys [hands] :as game-state}]
  (let [[first-hand & hands] hands]
    (assoc game-state :hands (into (vec hands) [first-hand]))))

(defn play-rounds [{:keys [bomb hands] :as game-state} take-turn]
  (let [[my-hand & hands] hands
        nplayers (count hands)
        ncards (cards-per-player nplayers)]
    (if (and (< 0 bomb) (= ncards (count my-hand)))
      (recur (-> game-state take-turn draw-cards next-hand) take-turn)
      game-state)))
