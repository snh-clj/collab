...
(defn four-clojure-60
  ...
  lazy."
  ([f coll]
   (four-clojure-60 f (first coll) (rest coll)))
  ([f init coll]
   (lazy-seq
     (when-let [s (seq coll)]
       (let [i (if (nil? init) (first s) (f init (first s)))]
         (cons i (four-clojure-60 f i (next coll))))))

   ;;; Non-lazy loop-recur version.
   #_(loop
     [rv coll
      rval (list init)]
     (if (empty? rv)
       ...
       (recur
         (next rv)
         (cons (f (first rval) (first rv)) rval))))))
